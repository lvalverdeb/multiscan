...
|---|---|
| `VulnerableDependency` | package purl (type, namespace, name, version), canonical vulnerability id (CVE alias if the advisory carries one, else advisory ID — ADR 0012) *(Amended by ADR 0023 — the CVE is taken from `aliases`, else `upstream`, else `related`, so distro advisories that name their CVE outside `aliases` key on it too)*, normalized lockfile/manifest path |
| `ContainerVulnerability` | package purl (type, namespace, name, version), canonical vulnerability id (as above, ADR 0012, amended by ADR 0023), image digest |

...
| Q-12 | swc's TypeScript grammar backtracks exponentially: a 120-byte `.ts` file parses in **30 s** (`NFR-001` budgets 10 s for a 100k-LOC repo). Raise upstream, pin around a fix, bound with a worker-thread timeout, or accept and document? | **Documented, unfixed.** Reproducers and measurements in `testdata/corpus/sast-pathological/`. Found by the `T-704` fuzz soak; not JSX ambiguity (tested with `tsx: false` — equally slow). Confined to TypeScript; Python and JS are unaffected. `MAX_SOURCE_BYTES` cannot bound it (the inputs are ~120 bytes) and `deadline` only degrades the scan to `Partial` after the fact. Release-relevant for scanning untrusted repositories. |

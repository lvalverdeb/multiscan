...
| Q-14 | Should importing an SBOM with no vulnerability records resolve its components against the pinned snapshot, rather than yielding nothing? | **Open; conservative behaviour shipped.** CycloneDX imports its `vulnerabilities[]` (VEX) section and SPDX its `SECURITY`/`advisory` external refs. An SBOM carrying neither yields no Findings — correct, since the file records what is present, not what is wrong with it. Resolving components against the snapshot would be genuinely useful but is an **Engine's** job (producing findings from evidence), not a **Bridge's** (normalizing findings another tool produced), so it is not smuggled in here. |
| Q-13 | Should `MS-PAT-1` gain multi-statement (statement-subsequence) leaf patterns? | **Resolved by ADR 0019: yes.** It was the largest single cause of compile failures in the first real corpus (64 of 135). Working rules went 61 → 80 with no corpus or translator change. Ordering and co-occurrence only — still not dataflow (`NG-2`). |

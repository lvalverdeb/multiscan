...
| Q-12 | swc's TypeScript grammar backtracks exponentially. A 203-byte `.ts` file **did not finish parsing in 10 minutes**; a 120-byte one takes 30 s (`NFR-001` budgets 10 s for a whole 100k-LOC repo). Raise upstream, pin around a fix, bound with a worker-thread timeout, or accept the exposure? | **Documented, unfixed — treat as release-blocking for TypeScript scanning.** A 203-byte file committed to a repo hangs any scan of it, reachable by anyone who can open a pull request. Reproducers and measurements in `testdata/corpus/sast-pathological/`. Found by the `T-704` fuzz soak, which aborted on a hard timeout. Not JSX ambiguity (tested with `tsx: false` — equally slow). Confined to TypeScript; Python and JS are unaffected. Nothing on our side bounds it: `MAX_SOURCE_BYTES` is irrelevant at 203 bytes, and `deadline`/`cancel` are only read between files, so on the unbounded input control never returns. |
